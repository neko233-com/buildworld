        ...
        <div className="notification-popover-list">
          ...
          {feed.items.map(item => {
            ...
            return <button type="button" key={item.id} className={`web-notification-item ${copy.payload.status || 'pending'}`} onClick={() => openBuild(item)}>
              ...
              <span><strong>{copy.title}</strong><small>{copy.event} · {copy.status}{copy.payload.branch ? ` · ${copy.payload.branch}` : ''}</small><time>{new Date(item.created_at).toLocaleString()}</time></span>
            </button>
          })}
        </div>
      </section>}
