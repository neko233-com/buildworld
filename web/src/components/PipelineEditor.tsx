...
import { Braces, Check, CirclePlay, Code2, Copy, FileCode2, RotateCcw } from 'lucide-react'
import { api } from '../api'
import { useI18n } from '../i18n'
import { isTypeScriptPipelineSource, isJenkinsfilePipelineSource, prettyPipelineSource } from '../lib/configFormat'
...
const YAML_TEMPLATE = `name: Build
jobs:
  build:
    steps:
      - run: echo building
...
const TS_TEMPLATE = `import { definePipeline, shell, stage } from '@buildworld/pipeline'
...
const initialDocument = YAML_TEMPLATE
...
export default function PipelineEditor({ project, projects, onProjectChange, onSave, onRun, embedded = false }: PipelineEditorProps) {
  ...
  const format = isJenkinsfilePipelineSource(document) ? 'Jenkinsfile' : isTypeScriptPipelineSource(document) ? 'TypeScript' : 'YAML'
  ...
  const switchFormat = async (next: 'yaml' | 'ts' | 'jenkinsfile') => {
    const current: 'yaml' | 'ts' | 'jenkinsfile' = format === 'TypeScript' ? 'ts' : format === 'Jenkinsfile' ? 'jenkinsfile' : 'yaml'
    if (next === current) return
    const currentTemplate = current === 'yaml' ? YAML_TEMPLATE : current === 'ts' ? TS_TEMPLATE : JENKINSFILE_TEMPLATE
    const nextTemplate = next === 'yaml' ? YAML_TEMPLATE : next === 'ts' ? TS_TEMPLATE : JENKINSFILE_TEMPLATE
    const trimmed = document.trim()
    if (trimmed !== '' && trimmed !== currentTemplate.trim() && isDirty) {
      if (!await dialogs.confirm(t('pipeline.discardMessage'), { title: t('pipeline.discardTitle'), action: t('pipeline.discardAction') })) return
    }
    setDocument(nextTemplate)
    setMessage('')
    setValidation(pendingPipelineValidation(nextTemplate))
  }
  ...
  return <motion.section className={`pipeline-workbench pipeline-code-workbench ${embedded ? 'pipeline-workbench-embedded' : ''}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
    <header className="pipeline-heading">
      ...
      <div className="pipeline-actions">
        <div className="definition-toolbar-actions" role="group" aria-label={t('projects.sourceFormat')}>
          <button type="button" className={format === 'YAML' ? 'selected' : ''} onClick={() => switchFormat('yaml')}><FileCode2 size={14} />YAML</button>
          <button type="button" className={format === 'TypeScript' ? 'selected' : ''} onClick={() => switchFormat('ts')}><Code2 size={14} />TypeScript</button>
          <button type="button" className={format === 'Jenkinsfile' ? 'selected' : ''} onClick={() => switchFormat('jenkinsfile')}><FileCode2 size={14} />Jenkinsfile</button>
        </div>
        ...
        <button className="secondary-button" type="button" onClick={formatDocument} disabled={formatting}><Braces size={15} />{formatting ? t('config.formatting') : format === 'TypeScript' ? t('config.validate') : format === 'Jenkinsfile' ? t('config.validate') : t('config.format')}</button>
    ...
    <section className="pipeline-source-panel pipeline-source-only">
      <div className="panel-header"><div><FileCode2 size={16} />{t('projectDetail.pipelineConfig')}</div><span>{format === 'TypeScript' ? t('config.typescriptHelp') : format === 'Jenkinsfile' ? t('config.jenkinsfileHelp') : t('config.yamlHelp')}</span></div>
