...
// isJenkinsfilePipelineSource mirrors the backend detection so the editor can
// highlight and switch formats consistently with the server-side parser.
export function isJenkinsfilePipelineSource(source: string) {
  const trimmed = source.trim()
  if (!trimmed) return false
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) return false
  if (trimmed.startsWith('#')) return false
  if (trimmed.includes('pipeline {') || trimmed.includes('node {')) return true
  if (trimmed.includes('stage(') && (trimmed.includes('steps') || trimmed.includes('sh ') || trimmed.includes('echo ') || trimmed.includes('script '))) {
    return true
  }
  return false
}

export type PipelineSourceLanguage = 'typescript' | 'yaml' | 'jenkinsfile'
...
export function resolvePipelineSourceLanguage(source: string, fallback: PipelineSourceLanguage = 'yaml'): PipelineSourceLanguage {
  ...
  if (isJenkinsfilePipelineSource(source)) return 'jenkinsfile'
  if (/^jobs\s*:/m.test(source)) return 'yaml'
  return fallback
}

function ensureSupportedPipelineSource(source: string): PipelineSourceLanguage {
  const trimmed = source.trim()
  if (!trimmed) throw new Error('Configuration cannot be empty')
  if (isTypeScriptPipelineSource(source)) return 'typescript'
  if (isJenkinsfilePipelineSource(source)) return 'jenkinsfile'
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
    throw new Error('JSON pipeline configs are not supported. Use TypeScript, YAML, or Jenkinsfile.')
  }
  if (trimmed.startsWith('#')) {
    throw new Error('Markdown pipeline configs are not supported. Use TypeScript, YAML, or Jenkinsfile.')
  }
  return 'yaml'
}

export const JENKINSFILE_TEMPLATE = `pipeline {
  agent any
  stages {
    stage('Build') {
      steps {
...
export function prettyConfigSourceSync(source: string): string {
  ...
  if (isJenkinsfilePipelineSource(source)) return `${trimmed}\n`
...
export async function prettyPipelineSource(source: string): Promise<string> {
  const trimmed = source.trim()
  const format = ensureSupportedPipelineSource(source)
  if (format === 'typescript' || format === 'jenkinsfile') return `${trimmed}\n`
  const { parseDocument } = await import('yaml')
  const document = parseDocument(trimmed)
  if (document.errors.length) throw document.errors[0]
