...
describe('requested UI contracts', () => {
  ...
  it('applies the final Jenkins page cascade to the desktop rail and viewport height', () => {
    ...
    expect(getComputedStyle(home).gridTemplateColumns).toBe('300px minmax(0, 1fr)')
    expect(getComputedStyle(home).minHeight).toBe('calc(100vh - 66px)')
    expect(getComputedStyle(rail).width).toBe('300px')
  ...
  it('collapses the Jenkins home layout cleanly at tablet and phone widths', () => {
    ...
    expect(tabletStyles).toMatch(/\.jenkins-home\s*\{[^}]*grid-template-columns:\s*300px minmax\(0,\s*1fr\)/)
    expect(tabletStyles).toMatch(/\.jenkins-rail-root\s*\{[^}]*width:\s*300px/)
