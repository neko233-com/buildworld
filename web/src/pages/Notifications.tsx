...
export default function Notifications() {
  ...
  return <>{breadcrumb}<section className="notification-workbench jenkins-management-page">
    ...
    {eventsChannel && <ModalDialog className="notification-events-modal" ariaLabel={t('notifications.deliveryHistory')} closeOnBackdrop onClose={() => setEventsChannel(null)}><header><div><Clock3 size={18} /><div><h2>{eventsChannel.name}</h2><p>{t('notifications.eventsDescription')}</p></div></div><button onClick={() => setEventsChannel(null)} title={t('common.close')} aria-label={t('common.close')}><X size={18} /></button></header><div className="notification-events-list">{events.length ? events.map(event => <article key={event.id} className={`notification-event ${event.status}`}><div><strong>{labelForEvent(event.event_type)}</strong><small>{formatDateTime(event.created_at)}</small></div><span>{labelForDelivery(event.status)}</span>{event.error_message && <p>{event.error_message}</p>}</article>) : <p className="detail-empty">{t('notifications.noDeliveryAttempts')}</p>}</div><footer><button data-dialog-initial-focus onClick={() => setEventsChannel(null)}>{t('common.close')}</button></footer></ModalDialog>}
  </section></>
}
