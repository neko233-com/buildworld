...
import { projectGroupPath, sortProjectGroups } from '../lib/projectGroups'
...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    ...
    try { config = prettyPipelineSourceSync(config) } catch { /* Surface validation when the user formats or saves. */ }
    setForm({ name: project.name, description: project.description || '', repo_url: project.repo_url || '', repo_type: 'git', default_branch: project.default_branch, vcs_root_id: project.vcs_root_id ?? null, template_id: project.template_id ?? null, group_id: project.group_id ?? null, tags: (project.tags || []).join(', '), config })
  ...
  const handleBuildNow = async () => {
    setBuilding(true)
    try {
      const metadata = await api.validatePipeline(project?.config || '')
  ...
  const handleFormatConfig = async () => {
    ...
    try {
      const config = typeScriptPipeline ? form.config : await prettyPipelineSource(form.config)
      await api.validatePipeline(config)
      setForm((current: any) => ({ ...current, config }))
      if (typeScriptPipeline && (!pipelineValidation.diagnosticsReady || pipelineValidation.diagnosticErrors > 0 || pipelineValidation.source !== config)) {
        throw new Error(pipelineBlockedMessage)
      }
      setNotice(typeScriptPipeline ? t('config.validated') : t('config.formatted'))
      dialogs.notify(typeScriptPipeline ? t('config.validated') : t('config.formatted'), 'success')
    } catch (reason: any) {
  ...
  const savePipeline = async (document: string) => {
    await api.validatePipeline(document)
  ...
  return <motion.section className="detail-page project-detail-page" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24, ease: 'easeOut' }}>
    ...
    <section className="detail-panel project-workbench"><nav className="detail-tabs" aria-label={t('projectDetail.views')}><button className={activeTab === 'overview' ? 'active' : ''} onClick={() => showView('overview')}><FileCode2 size={15} />{t('projectDetail.overview')}</button><button className={activeTab === 'builds' ? 'active' : ''} onClick={() => showView('builds')}><Activity size={15} />{t('builds.title')}<span>{buildList.length}</span></button><button className={activeTab === 'build' ? 'active' : ''} onClick={() => showView('build')}><Workflow size={15} />{t('projectDetail.buildSteps')}</button><button className={activeTab === 'settings' ? 'active' : ''} onClick={() => showView('settings')}><Settings2 size={15} />{t('projects.settings')}</button></nav>
      <div className="detail-panel-body">
        {activeTab === 'overview' && <div className="pipeline-preview"><header><div><FileCode2 size={16} /><h2>{t('projectDetail.pipelineConfig')}</h2></div><button className="table-link" onClick={() => showView('build')}><Workflow size={14} />{t('projectDetail.openBuildSteps')}</button></header><pre>{(() => { try { return prettyPipelineSourceSync(project.config || '') } catch { return project.config || '' } })()}</pre></div>}
        ...
        {activeTab === 'settings' && form && <div className="project-settings-stack">
          <section className="project-config-section">
            ...
            <form onSubmit={handleSave} className="project-settings-form">
              ...
              <div><label htmlFor="project-settings-repository-type">{t('projects.repositoryType')}</label><input id="project-settings-repository-type" type="text" readOnly value={t('projects.gitOnly')} aria-readonly="true" /></div>
              ...
              <div><label htmlFor="project-settings-group">{t('projectGroups.title')}</label><select id="project-settings-group" disabled={!editable} value={form.group_id ?? ''} onChange={set('group_id')}><option value="">{t('projectGroups.ungrouped')}</option>{groups.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select><small>{t('projectGroups.assignmentHelp')}</small></div>
              ...
              {!typeScriptPipeline && <div className="settings-wide"><ApprovalSettingsEditor source={form.config} disabled={!editable} onChange={config => { setPipelineValidation(pendingPipelineValidation(config)); setForm((current: any) => ({ ...current, config })) }} onError={setFormError} /></div>}
              <div className="settings-wide config-editor-label"><span><label id="project-settings-pipeline-label">{t('projectDetail.pipelineConfig')}</label>{editable && <div className="config-editor-actions"><button type="button" className="format-command" onClick={() => setShowJenkinsImport(true)}><FileInput size={13} />{t('jenkinsImport.title')}</button><button type="button" className="format-command" onClick={handleFormatConfig} disabled={formatting}><Braces size={13} />{formatting ? t('config.formatting') : typeScriptPipeline ? t('config.validate') : t('config.format')}</button></div>}</span><PipelineSourceEditor ariaLabel={t('pipeline.sourceLabel')} readOnly={!editable} value={form.config} onChange={config => { setPipelineValidation(pendingPipelineValidation(config)); setForm((current: any) => ({ ...current, config })) }} onValidationChange={setPipelineValidation} statusId="project-settings-pipeline-status" height={420} /><small>{typeScriptPipeline ? t('config.typescriptHelp') : t('config.yamlHelp')}</small></div>
              {formError && <p className="form-error">{formError}</p>}
              {editable && <footer><button type="submit" className="primary-command" disabled={saving || !pipelineReady} aria-describedby={!pipelineReady ? 'project-settings-pipeline-status' : undefined} title={!pipelineReady ? pipelineBlockedMessage : undefined}>{saving ? t('common.loading') : t('common.save')}</button><button type="button" className="danger-command" disabled={deleting} onClick={handleDelete}><Trash2 size={14} />{deleting ? t('common.loading') : t('projects.delete')}</button></footer>}
            </form>
          </section>
        </div>}
      </div>
